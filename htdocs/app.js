(function () {
    ...
    function bridgeAvailable() { return typeof window.AndroidBridge !== 'undefined'; }
    ...
    window.BlueMagicNative = {
        ...
        onCommandStatus: function (data) {
            ...
            if (data.command) { upsertCommand(data.command); renderCommands(); }
        }
    };
    ...
    function initialize() {
        ...
        try { configuration = JSON.parse(window.AndroidBridge.getConfiguration() || '{}'); }
        catch (ignored) { configuration = {}; }
        ...
        if (configuration.role === 'DSM' || configuration.role === 'POS') byId('requestSupplyCard').classList.remove('hidden');
        if (configuration.role === 'DAE' || configuration.role === 'DSM') byId('supplyChildCard').classList.remove('hidden');
        if (configuration.role === 'POS') byId('retailCard').classList.remove('hidden');
        bindActions(); renderCommands(); refreshCommands();
        window.setInterval(function () { if (document.visibilityState === 'visible') refreshCommands(); }, 15000);
    }

    function bindActions() {
        each('button[data-action]', function (button) {
            button.addEventListener('click', function () { execute(button.getAttribute('data-action')); });
        });
    ...
    function execute(action) {
        ...
        var requestType = '', targetNode = '', targetPhone = '', amount = '';
        if (action === 'request-supply') { requestType = 'REQUEST_SUPPLY'; amount = byId('requestSupplyAmount').value.trim(); }
        else if (action === 'supply-child') { requestType = 'SUPPLY_CHILD'; targetNode = byId('childNode').value.trim().toUpperCase(); amount = byId('childAmount').value.trim(); }
        else if (action === 'retail-sale') { requestType = 'RETAIL_SALE'; targetPhone = byId('retailPhone').value.replace(/\D/g, ''); amount = byId('retailAmount').value.trim(); }
        else if (action === 'test-number') requestType = 'TEST_NUMBER';
        if (requestType !== 'TEST_NUMBER' && !/^[1-9]\d{0,8}$/.test(amount)) return showToast('Saisissez un montant entier valide.');
    ...
    function refreshCommands() {
        ...
        for (var index = 0; index < commands.length; index += 1) {
            if (!TERMINAL[commands[index].state]) window.AndroidBridge.getCommandStatus(commands[index].public_id);
        }
    }
    ...
    function upsertCommand(command) {
        ...
        for (var position = 0; position < commands.length; position += 1) {
            if (commands[position].public_id === command.public_id) { index = position; break; }
        }
        if (index >= 0) {
            for (var key in command) if (Object.prototype.hasOwnProperty.call(command, key)) commands[index][key] = command[key];
    ...
    function commandStorageKey() { return 'blue_magic_recent_commands_v4_' + (configuration.profile_id || configuration.node_code || 'default'); }
    function renderCommands() {
        if (!commands.length) { byId('commandList').innerHTML = '<p class="muted">Aucune commande sur ce téléphone.</p>'; return; }
        var html = '';
        for (var index = 0; index < commands.length; index += 1) {
            ...
            html += '<article class="command-item"><div class="command-item-top">'
                ...
                + (timestamp ? '<div class="command-time">Horodatage : ' + escapeHtml(formatTimestamp(timestamp)) + '</div>' : '') + '</article>';
        }
    ...
    function formatTimestamp(value) {
        ...
        return two(date.getDate()) + '/' + two(date.getMonth() + 1) + '/' + date.getFullYear() + ' à '
    ...
    function showToast(message) {
        var toast = byId('toast'); toast.textContent = message; toast.classList.remove('hidden');
