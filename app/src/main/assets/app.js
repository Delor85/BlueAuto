(function () {
    ...
    var labels = {PENDING:'En attente',LEASED:'Réservée au Robot',DIALING:'Composition USSD',AWAITING_PIN:'Vérification du pop-up',PIN_SUBMITTED:'PIN validé',AWAITING_RESULT:'Confirmation Camtel',SUCCEEDED:'Réussie',FAILED:'Échec',UNKNOWN:'À vérifier',BLOCKED:'Robot bloqué — PIN'};
    ...
    function initialize(){
        ...
        if(configuration.pin_blocked){byId('fatalNotice').textContent='PIN BLOQUÉ : corrigez le PIN avant toute reprise.';byId('fatalNotice').className='notice notice-danger';}
    ...
    function execute(action){
        if(!bridge()||configuration.pin_blocked){return;}var type='',node='',phone='',amount='';
