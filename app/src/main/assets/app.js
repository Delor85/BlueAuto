(function () {
    ...
    window.BlueMagicNative={
        ...
        onCommandCreated:function(data){var submitted=submittedForm;if(submitted&&submitted.type==='SUPPLY_CHILD'&&window.BIRCommissionPolicy&&window.BIRCommissionPolicy.clearOneShotRate){window.BIRCommissionPolicy.clearOneShotRate(submitted.node);}setBusy(false);pendingPreview=null;submittedForm=null;if(data.error){showActionError(data);return;}clearActionError();var command=data.command||{};if(!command.public_id){showActionError({code:'INVALID_RESPONSE',message:'Réponse serveur incomplète.'});return;}clearSubmittedFields(submitted);upsert(command);render();try{if(window.AndroidBridge.kickSynchronization){window.AndroidBridge.kickSynchronization();}}catch(ignored){}scheduleCommandPoll(1200);if(command.robot_ready===false){showActionProgress((command.robot_status||'ROBOT_OFFLINE')+' — '+(command.robot_message||'Commande créée, mais le Robot ne communique pas encore.'));showToast('Commande créée, mais Robot hors ligne.');return;}showActionProgress('Commande créée. Le Robot détenteur de la SIM est connecté et va la prendre.');showToast(data.duplicate?'Cette demande existait déjà.':'Commande créée et transmise au Robot exact.');},
        onCommandStatus:function(data){if(data&&data.command){upsert(data.command);render();if(TERMINAL[data.command.state]){try{if(window.AndroidBridge.kickSynchronization){window.AndroidBridge.kickSynchronization();}}catch(ignored){}requestDashboard(true);}scheduleCommandPoll(5000);}},
