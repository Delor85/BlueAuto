...
must(gradle.includes('versionCode 57') && gradle.includes('versionName "2.9.2"'), 'v2.9.2 identity missing');
must(index.includes('pilotage-v292.css') && index.includes('pilotage-v292.js'), 'v2.9.2 dashboard assets not loaded');
must((api.match(/app_version", "2\.9\.2/g) || []).length >= 2, 'native heartbeat/activation does not report 2.9.2');
must(api.includes('|ops_cockpit|ops_assist|ops_escalate|'), 'ops_assist is not allowed through native bridge');
must(worker.includes("const API_VERSION = '2.9.2-cloudflare'"), 'Worker version mismatch');
