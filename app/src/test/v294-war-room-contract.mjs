...
need(war.includes('Stock hors commission')&&war.includes('Commission incluse')&&war.includes('Total Blue certifié'),'DAE/DSM three-balance semantics missing');
need(war.includes("balance_reusable===true")&&war.includes("balance_quality==='EXACT'"),'uncertain balances must be hidden');
need(war.includes("main.insertBefore(host,main.firstChild)"),'War Room must remain at the very top');
...
need(api.includes('payload.put("app_version", BuildConfig.VERSION_NAME);')||api.includes('payload.put("app_version", "2.9.4");')||api.includes('payload.put("app_version", "2.9.5");')||api.includes('payload.put("app_version", "2.9.6");')||api.includes('payload.put("app_version", "2.9.7");'),'v2.9.4+ telemetry missing');
